##############################################
# Kleaf Model Fitting Script
# Multi-species version (looped by species × data.type which is here season)
#for simplicity, 'kleaf' has been renamed 'gs' in the data csv file.
##############################################
...
do_the_thing_nonlinear <- function(
  ...
  return(list(
    summary = parvec, # what goes to Excel
...
input_file <- "data/working/plasticitykleaf_reformat.csv"
...
write.csv(
  all_results_df,
  file.path(output_dir, "model_fitting_results_kleaf.csv"),
  row.names = FALSE
)
saveRDS(all_results_r, file.path(output_dir, "model_fitting_results_full_kleaf.RDS"))
